...
class UserDatabaseHelper {
  ...
  Future<Database> _initDB(String filePath) async {
    final dbPath =
        await getDatabasesPath(); //Quy định thư mục của CSDL được lưu trữ trên điện thoại
    ...
    return await openDatabase(
      path,
      version: 1,
      //CSDL được tạo và chạy lần đầu tiên
      onCreate: _createDB,
    );
  }
  ...
  Future _insertSampleData(Database db) async {
    ...
    for (final userData in sampleUsers) {
      await db.insert('users', userData); //userData là một Map (record)
    }
  }
